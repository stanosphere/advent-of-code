...
part2 :: IO Int
part2 = countGarbage . unsafeParse groupParser <$> getInput

getInput :: IO String
getInput = readFile "./fixtures/input9.txt"

countGarbage :: Group -> Int
countGarbage = sum . map countGarbageInSubGroup
  where
    countGarbageInSubGroup :: GroupItem -> Int
    countGarbageInSubGroup (Garbage str) = length str
    countGarbageInSubGroup (SubGroup sg) = countGarbage sg
...
groupParser = P.char '{' *> groupItemParser `sepBy` P.char ',' <* P.char '}'
  where
    ...
    garbageParser = Garbage . catMaybes <$> (P.char '<' *> P.many (normalChar P.<|> escapedChar) <* P.char '>')
      where
        normalChar = Just <$> P.noneOf "!>"
        escapedChar = Nothing <$ (P.char '!' *> P.anyChar)
