...
validsMemo xss yss = valids xss yss
  where
    lx = length xss
    ly = length yss
    vals =
      A.array
        ((0, 0), (length xss, length yss)) -- Maps from dropped chars to actual result
    ...
    validsO xs@('#' : _) (y : ys) =
      if length xs >= y -- there have to be at least y springs to be y damaged springs
...
main :: IO ()
main = do
  input <- map parse <$> getLines "./fixtures/input12.txt"
